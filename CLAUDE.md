...
- **Update** (`_Update_Continuous_Voice_Profile`): runs once per voiced timepoint (`timepoint_is_voiced`) during a turn where the active speaker is the voice_id itself. First, `voice_timepoints_count` is incremented. The update math then branches per the active version (mutually exclusive — see `"nudge_step_version"`/`"divergence_scaling_version"` above):
  ...
  In the two estimating versions, the update happens before this same timepoint is scored, so a voice's own-turn comparisons always use its freshly updated profile (the same is true of `perfect_tracking_version`'s insert). After the per-point updates, those two versions clamp the three points' `projected_distribution_ratio` values back into a valid `lower_standard_deviation ≤ median ≤ upper_standard_deviation` order (they're hunted independently with no ordering guarantee, which could otherwise let lower/upper cross the median and produce a degenerate spread). `perfect_tracking_version` needs no such clamp: reading three ascending target percentiles off one ascending record yields ordered ratios by construction.

- **Reading the profile** (`_Continuous_Voice_Profile_Point_Ratios`): the single place all three versions' state is turned back into the three points' distribution ratios, so `_Continuous_Voice_Profile_Bell_Curve_Projections` and `_Continuous_Voice_Profile_Convergence_Values` can never disagree about what the profile currently says. For the two estimating versions it returns each point's `projected_distribution_ratio` directly; for `perfect_tracking_version` it calls `_Perfect_Tracking_Ratio_At_Percentile(sorted_bucket_ratios, target_percentile)`, which returns the record entry at rank `round(target_percentile * record_length)` (clamped into range; `0.0` if the record is empty). That rank lookup is exactly equivalent to the "observed ratio whose occurrence percentile is closest to the target" choice `_Extract_Bell_Curve_Projections` makes against a persisted JSON — verified to match it to the bit on identical observations — which is what makes this version an apples-to-apples ideal baseline for the convergence chart rather than a differently-defined quantity.
