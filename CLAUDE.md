...
- `cross_type_hyperparameters`: parameters shared across all variants:
  ...
  - `"continuous_voice_profiling"`: `dict` — see **Continuous voice profiling** below. Sub-keys:
    ...
    - `"divergence_scaling_version"`: `dict` — the alternative logic version, which scales its update size by how far `cumulative_occurrence_percentile` currently is from its target instead of always stepping by a fixed amount. Sub-keys: `"use_version"` (bool, default `False`); `"occurrence_percentile_cumulation_update_power"` (float); `"distribution_ratio_cumulation_update_power"` (float); `"divergence_scaling_power"` (float); `"cumulation_weight_initialization_base"` (int). If `"use_version"` is `True` for both `nudge_step_version` and `divergence_scaling_version`, `Run_Element_Match_Contribution_Type_Exploration` raises a `ValueError` and the run is aborted — the only hard-error misconfiguration in this file, as opposed to every other option's warn-and-ignore treatment. If `use_continuous_voice_profiling` is `True` but neither version's `"use_version"` is `True`, a warning is printed and continuous voice profiling is ignored for the run.
...
- **State**: tracked separately per voice_id (reset per `_Process_Sequences_For_Voice` call) and, within that, per frequency bucket per bell curve point (`lower_standard_deviation` target percentile `0.15865`, `median` target percentile `0.5`, `upper_standard_deviation` target percentile `0.84135`). A voice-level `voice_timepoints_count` starts at `0`; each bucket/point pair tracks `projected_distribution_ratio` (starting at `1.0 / bucket_count`, an even split across buckets) and `cumulative_occurrence_percentile` (starting at that point's own `target_percentile` — both starting points chosen as a neutral "no evidence yet" state rather than a value the first few updates have to climb out of). Whether this state persists across sequences or resets each sequence is controlled by `continue_voice_profiles_across_conversations` (see above). Both starting values and this state shape are shared by `nudge_step_version` and `divergence_scaling_version` alike — only the update math below differs between them.
- **Update** (`_Update_Continuous_Voice_Profile`): runs once per voiced timepoint (`timepoint_is_voiced`) during a turn where the active speaker is the voice_id itself. First, `voice_timepoints_count` is incremented. The update math then branches per the active version (mutually exclusive — see `"nudge_step_version"`/`"divergence_scaling_version"` above):
  ...
  In both versions, the update happens before this same timepoint is scored, so a voice's own-turn comparisons always use its freshly updated profile. After the per-point updates, the three points' `projected_distribution_ratio` values are clamped back into a valid `lower_standard_deviation ≤ median ≤ upper_standard_deviation` order (they're hunted independently with no ordering guarantee, which could otherwise let lower/upper cross the median and produce a degenerate spread) — this clamp step is unconditional and identical for both versions.
